...
public class SpecialiteDto {
	...
	private List<CoursLightDTO> courses; 
	...
	public SpecialiteDto(Long id, String labelle, String image,DiciplineDTO dicipline, List<CoursLightDTO> courses) {
		super();
		this.id = id;
	...
	public static SpecialiteDto map(Specialite specialite) {
		return specialite == null ? null : new SpecialiteDto(specialite.getId(),specialite.getLabelle(),specialite.getImage()
				...
				,specialite.getCoursList()==null ?null : specialite.getCoursList().stream().map(CoursLightDTO::map).collect(Collectors.toList()));
	}
	...
	public List<CoursLightDTO> getCourses() {
		return courses;
	}


	public void setCourses(List<CoursLightDTO> courses) {
		this.courses = courses;
	}
